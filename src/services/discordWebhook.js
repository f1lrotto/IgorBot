...
const config = {
  news: {
    ...
    username: "Memečiar",
    avatarURL:
      "https://cdn.discordapp.com/attachments/457885524292665348/1154872367995162634/image.png",
  },
  train: {
  ...
  formula: {
    ...
    username: "hihi:)",
    avatarURL:
      "https://media.discordapp.net/attachments/1000236490133090355/1168283077408211025/F1-Instagram-1280x720.jpg?ex=65513367&is=653ebe67&hm=bdabe2ab1982e7d0916921c8e11a96bac539305787f71c09dfbd60b6cbc19836&=&width=1202&height=676",
  },
};
...
const articleCategoryConfig = {
  ...
  Šport: {
    ...
    iconURL:
      "https://cdn.discordapp.com/attachments/457885524292665348/1154879086351630416/image.png",
  },
};
...
async function sendDiscordMessage(clientConfig, items, createEmbed) {
  ...
  for (let i = 0; i < embeds.length; i += 10) {
    ...
    console.info(
      `Sending ${batch.length} embeds in batch ${Math.floor(i / 10) + 1}`
    );
...
function createNewsEmbed(article) {
  ...
  const combinedDateTime = `${moment(article.articleDate, "DD. M. YYYY").format(
    "DD/MM/YYYY"
  )} ${article.articleTime}`;
  ...
  return new EmbedBuilder()
    ...
    .setAuthor({
      ...
      url: categoryConfig.categoryUrl,
...
function createFormulaEmbed(article) {
  return new EmbedBuilder()
    ...
    .setAuthor({
      name: article.tag,
      url: "https://www.formula1.com/en/latest/all.html#default",
      iconURL: config.formula.avatarURL,
    })
...
module.exports = {
  sendNewsDiscordMessage: async (articles) =>
    sendDiscordMessage("news", articles, createNewsEmbed),
  sendTrainDiscordMessage: async (trains) =>
    sendDiscordMessage("train", trains, createTrainEmbed),
  sendFormulaDiscordMessage: async (articles) =>
    sendDiscordMessage("formula", articles, createFormulaEmbed),
};
