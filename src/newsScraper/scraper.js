...
const scrapeOverview = async (url) => {
  ...
  try {
    $(".js-article-short-list-wrapper > div").each((i, el) => {
      ...
      const dateElement = mainDiv.find(
        ".article-short-side-content div a.fs-16"
      );
      let articleDate =
        dateElement.length > 0
          ? dateElement.text()
          : moment().tz("Europe/Bratislava").format("D. M. YYYY");

      const articleTime = mainDiv
        .find(".sans-bold.fs-16.article-short-time a")
        .text();
      ...
      const headlineElementA = mainDiv.find(
        ".article-short-content p a strong"
      );
      const headlineElementStrong = mainDiv.find(
        ".article-short-content p strong:not(:has(a))"
      ); // strong tag not containing anchor tag
      let headline;
      ...
      console.info(`Scraped article ${articleId}-${headline}`);
